...
<!-- consumer-specific: asset-studio context-systems roster (KOS). Replace per consumer;
     slugs here are the canonical `kos` tag vocabulary for this project's sessions. -->
| slug | system | reach for it when |
|---|---|---|
| assets-library | the shared `~/Master-Managed/Assets/` library (referenced, never duplicated) | locating or placing produced assets |
| blender | Blender MCP + the blender-mcp-craft skill | 3D modeling / rendering production steps |
| figma | Figma design sources | UI / 2D design sources and exports |
| mediums | `_project/mediums/` capability catalog | what production capability exists per asset type |
| explorer | the Asset Explorer / Command Center (`previews/dashboards/*.json`) | current factual state of the asset library |
